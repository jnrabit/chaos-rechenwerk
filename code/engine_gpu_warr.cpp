// engine_gpu_mf.cpp — N Zellen Doppelkern-Lorenz + mean-field auf GPU (OpenCL, double/fp64).
// Zwei-Phasen-Tick: Host bildet mean-Schnappschuss (Tick-Anfang), GPU integriert alle N mit gefrorenem mean.
// Aufruf: ./gpu_mf <init.bin> <out.bin> <N> <nticks> <t0> <record0/1>
...
static S8 deriv(S8 s,double W,double sig,double rho,double bet,double K,double mx1,double mx2){
  ...
  d.x1 += K*(mx1 - s.x1);          d.x2 += K*(mx2 - s.x2);   // mean-field, gefrorener mean, alle 4 Stufen
  return d;
}
...
__kernel void run_tick(__global const double* in,__global double* out,__global double* xout,
                       int N,double W,double mx1,double mx2){
  int g=get_global_id(0); if(g>=N) return;
  const double SIG=10.0,RHO=28.0,BET=8.0/3.0,DT=0.005,K=0.05;
  S8 s; s.x1=in[g*8+0];s.y1=in[g*8+1];s.z1=in[g*8+2];s.w1=in[g*8+3];
        s.x2=in[g*8+4];s.y2=in[g*8+5];s.z2=in[g*8+6];s.w2=in[g*8+7];
  s = rk4(s,W,SIG,RHO,BET,DT,K,mx1,mx2);
  out[g*8+0]=s.x1;out[g*8+1]=s.y1;out[g*8+2]=s.z1;out[g*8+3]=s.w1;
  out[g*8+4]=s.x2;out[g*8+5]=s.y2;out[g*8+6]=s.z2;out[g*8+7]=s.w2;
  xout[g*2+0]=s.x1; xout[g*2+1]=s.x2;     // x1,x2 fuer Host-Mittel des naechsten Ticks
}
)CLC";
...
int main(int argc,char**argv){
  if(argc<7){fprintf(stderr,"usage: %s init.bin out.bin N nticks t0 record warr.bin\n",argv[0]);return 2;}
  ...
  cl_kernel k=clCreateKernel(prog,"run_tick",&err);

  std::vector<double> init=read_bin(argv[1]);   // N*8
  std::vector<double> warr = (argc>7)? read_bin(argv[7]) : std::vector<double>();
  ...
  cl_mem bX=clCreateBuffer(ctx,CL_MEM_READ_WRITE,N*2*sizeof(double),nullptr,&err);
  std::vector<double> xbuf(N*2), st(N*8), full;
  if(record) full.assign((size_t)N*nticks*8, 0.0);

  auto host_mean=[&](int i)->std::pair<double,double>{
    double sx1=0,sx2=0;
    if(i==0){ for(int n=0;n<N;n++){sx1+=init[n*8+0];sx2+=init[n*8+4];} }   // Tick 0: aus init
    else    { for(int n=0;n<N;n++){sx1+=xbuf[n*2+0];sx2+=xbuf[n*2+1];} }    // sonst: x vom Vortick (=Tick-Anfang)
    return {sx1/(double)N, sx2/(double)N}; };

  cl_mem in=bA, out=bB;
  auto T0=std::chrono::high_resolution_clock::now();
  for(int i=0;i<nticks;i++){
    double W = (i<(int)warr.size())? warr[i] : (0.225+0.018*std::sin(1.8*(t0+(double)i*DT)));
    std::pair<double,double> m=host_mean(i);
    clSetKernelArg(k,0,sizeof(cl_mem),&in); clSetKernelArg(k,1,sizeof(cl_mem),&out);
    clSetKernelArg(k,2,sizeof(cl_mem),&bX); clSetKernelArg(k,3,sizeof(int),&N);
    clSetKernelArg(k,4,sizeof(double),&W); clSetKernelArg(k,5,sizeof(double),&m.first); clSetKernelArg(k,6,sizeof(double),&m.second);
    size_t g=((size_t)(N+63)/64)*64;
    clEnqueueNDRangeKernel(q,k,1,nullptr,&g,nullptr,0,nullptr,nullptr);
    clEnqueueReadBuffer(q,bX,CL_TRUE,0,N*2*sizeof(double),xbuf.data(),0,nullptr,nullptr);
